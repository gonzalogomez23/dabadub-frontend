...
const PrimaryButton = ({ children, variant = 'primary', className = '', ...props }: PrimaryButtonProps) => {
  ...
  const styleClasses = `rounded-xl border border-2 transition-colors flex items-center justify-center cursor-pointer gap-2 font-medium text-sm sm:text-base h-10 sm:h-12 px-4 py-2 sm:px-5 ${className} ${variantClasses[variant]}`
