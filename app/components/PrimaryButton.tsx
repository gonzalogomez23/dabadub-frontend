...
const PrimaryButton = ({ children, variant = 'primary', className = '', ...props }: PrimaryButtonProps) => {
  ...
  const styleClasses = `rounded-full border border-solid border-transparent transition-colors flex items-center justify-center cursor-pointer gap-2 font-medium text-sm sm:text-base h-10 sm:h-12 px-4 sm:px-5 sm:w-auto ${className} ${variantClasses[variant]}`
